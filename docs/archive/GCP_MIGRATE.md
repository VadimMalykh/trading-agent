# Migrate local data → GCP (archived)

> **ARCHIVED 2026-09-04 — the one-off runbook for migrating local data to GCP, which was done.** Its one standing instruction — install the candle integrity guard on every instance that collects candles — moved into [CANDLE_GUARD.md](../CANDLE_GUARD.md) §3 before this file was archived.
> Kept for the day a second instance needs the same export/upload/import path; the scripts it calls still exist.
