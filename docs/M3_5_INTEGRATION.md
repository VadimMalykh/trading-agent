...
| `policy.skips` | Named reasons bars were not traded: `below_coverage`, `position_open`, `no_regime`, `no_side`, `not_served`, and the guard's four: `checkpoint_mismatch`, `checkpoint_unverified`, `interval_mismatch`, `interval_unverified`. ⚠️ `warming_up` should **never appear again** — if it does, the running binary predates the freeze. **A silent system with a populated `skips` map is working.** `not_served` counts bars for collected-but-not-traded pairs; since 2026-08-29 the collector and the served set hold the same twelve, so it should now be **zero** — a non-zero value means the two lists have drifted apart. |
