`served_candle_interval: null` means the running `serve.py` predates that fix. 🔴 Added 2026-09-11: the guard also requires `policy.served_closed_bars_only: true` — `serve.py` reporting that its features end on a **closed** candle. Before that it scored the still-forming bar and re-scored it every 30 s as the bar moved; the forward test's first trade opened on such a re-score ([M3_FIDELITY_RESULTS §7.6](./M3_FIDELITY_RESULTS.md)). `null` means an old `serve.py`. |# M3-5 — the rule, wired to the executor
...
| `policy.checkpoint_bound`, `policy.served_candle_interval` vs `policy.expected_candle_interval` | The binding guard (M3_PROTOCOL §9.5). `checkpoint_bound` must be `true`, and it is only `true` when **both** the served checkpoint hash equals `frozen_checkpoint` **and** the candle interval `ml_inference` builds features on equals `5m`. 🔴 Added 2026-09-10: from the 08-24 promotion to 09-10 the VM served the right weights from **1m** candles, the hash matched throughout, and every live confidence sat ~0.05 below the offline value on the same bars — the forward test took no trades ([M3_FIDELITY_RESULTS §7.5](./M3_FIDELITY_RESULTS.md)). `served_candle_interval: null` means the running `serve.py` predates that fix. |
| `policy.skips` | Named reasons bars were not traded: `below_coverage`, `position_open`, `no_regime`, `no_side`, `not_served`, and the guard's five: `checkpoint_mismatch`, `checkpoint_unverified`, `interval_mismatch`, `interval_unverified`, `forming_bar_unverified`. **`bar_already_recorded` is the largest count on a healthy system** (2026-09-11): the engine ticks every 30 s but decides each `(pair, bar_ts)` once, on the tick that put it in `policy_bars`, so roughly nine of every ten evaluations of a 5m bar land here by design. `bar_not_recorded` (the ledger insert failed, so the bar was not decided either) should be zero.
 ⚠️ `warming_up` should **never appear again** — if it does, the running binary predates the freeze. **A silent system with a populated `skips` map is working.** `not_served` counts bars for collected-but-not-traded pairs; since 2026-08-29 the collector and the served set hold the same twelve, so it should now be **zero** — a non-zero value means the two lists have drifted apart. |
