...
🔵 **2026-09-23: one offline lever registered — X8; all three runs DONE 2026-09-24, nothing read
yet — the read is the next fresh session's first job** (experiment-ideas table below; NEXT_TRAINING_PLAN §2). The M2 freeze's reopening condition — book/flow history inside the training window, written
...
Vadim: nothing** — the three runs are done and Claude reads them in a fresh session (row X8). The fetcher, loader and identity check were built in Docker, reading no model number. The forward test and both side arms
...
| **X8** | **Open-interest history inside the training window, from Binance's public archive** | The served model's `oi` / `oi_chg` legacy columns are zero in train, val and serve because the collector's OI starts 2026-07-18, after the train window ends. The public archive (`data.binance.vision`, USDⓈ-M `metrics`, 5m) carries the same series from 2020-09. Fill the two *existing* columns over the whole window; no column added, no serve feature code touched | GPU, serial: 3 runs (~2.5 h, ≈ $1.5 each, ~8 h); X0 is the banked control (same snapshot, same split). Before it: a fetcher, a loader union and an identity check archive-vs-collector on the 07-18 → 09-09 overlap — all CPU, in Docker | the M2 freeze's own reopening condition ("history inside the *training* window"), met by the archive rather than by the collector's calendar; the ≈2027 date was the collector's | 🔵 **ALL THREE RUNS DONE 2026-09-24, NOT YET READ — [NEXT_TRAINING_PLAN §2 X8](./NEXT_TRAINING_PLAN.md): s1 `20260923T160559Z`, s2 `20260923T204303Z`, s3 `20260924T024247Z` (early stops 37 / 36 / 34); logs `logs/X8_s{1,2,3}.log` and eval dumps on disk, acceptance lines verified on all three.** **Needed from Vadim: nothing.** **Claude does, in a fresh session:** the pre-registered read — §0.3's awk over the epoch lines of the three X8 logs and X0's three, the plateau-mean LB gate (±0.008, 15-epoch fallback), the cov-0.02 secondary, and the plateau-length reading that decides X8b; then the verdict block in §2 and this row. Prerequisites done 2026-09-23: archive fetched (sha8 83c85bd7), identity check PASS on all twelve pairs, acceptance lines verified on s1 and s2. Gate: X1's (±0.008 on the plateau-mean LB, 15-epoch fallback). Recorded expectation: FLAT; the plateau length is the reading that decides whether **X8b** (the long/short and taker ratios as a new 4-column group, written in the same section, not registered for launch) is worth three more runs. Depth bands from the archive are *not* registered: the collector's snapshot has no live counterpart for them |
