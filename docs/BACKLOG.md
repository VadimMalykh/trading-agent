...
🟢 **Mode as of 2026-09-20: waiting on the forward paper test, fourth start. Needed from Vadim:
nothing.** The served model is now the **twelve-pair** checkpoint U12 seed 2
(`m2_multi_20260916T164212Z_ace3ae5e.pt`), promoted 2026-09-20 and certified by the walk-forward
folds; every served pair is a trained pair and `/predict` refuses anything else (rows 10, 11).
The old ledger (34 trades, 31,412 bars, 77% of above-cut bars from untrained pairs) is void and
backed up. Claude runs the acceptance replay of the new start on the next session on or after
2026-09-21, then reads the ledger at every 50 closed trades (row 1).
...
| 1 | **The forward paper test** | [M3_5_INTEGRATION.md](./M3_5_INTEGRATION.md) | 🔵 **RUNNING since 2026-09-20 08:15:10 UTC — FOURTH START, on the twelve-pair checkpoint; acceptance replay PENDING.** **Needed from Vadim: nothing.** Do not touch the VM, the model or the rule. **Claude does, in the first session on or after 2026-09-21:** run `accept_76.py` on the first day's `policy_bars` rows (bar: every row exact to the stored 4 dp, as M3_FIDELITY_RESULTS §7.6) against checkpoint sha `30e6ac1e…` — the script (`ml/train/output/probe/accept_76.py`, gitignored) takes the checkpoint as `MODEL_PATH`, so pass `m2_multi_20260916T164212Z_ace3ae5e.pt` (in GCS `checkpoints/`), not the 0819 file its header names; the start stands only if it passes. **Then, on request:** (a) read `/api/health` on the VM (port 4000) and confirm: `checkpoint_bound: true`, closed bars only, 5m, `frozen_threshold` 0.6708709597587585, 12 served pairs, candle settle 120 s, `last_error: null`. (b) `./scripts/gcp_forward_ledger.sh` at every 50 closed policy trades — the pre-registered readings R0–R5 (M3_5 §4.3), per unit of notional; before 50 it prints TEXTURE and nothing may be quoted. The per-checkpoint constants (four cut levels, ladder, R5's 0.6179) were restated 2026-09-20 with the ledger empty. Mode `simulation`, 5.0-bps fee, $500 per size unit. Silence is normal: the longest dry spell ever measured is 21.5 days. *Background:* starts one to three (09-10 wrong bar size, 09-11 04:17 forming-bar scores, 09-11 06:07 → 09-20 on an eight-pair checkpoint serving twelve) are void; their ledgers are on the VM as `~/*_formingbar_20260911.csv`, `~/*_settle_20260911.csv`, `~/paper_trades_oov8ckpt_20260920.csv`, `~/policy_bars_oov8ckpt_20260920.csv` |
