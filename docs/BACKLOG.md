...
🟢 **Mode as of 2026-09-24: waiting on trades — paper test, exploratory arm and the real-money
micro-pilot all running. Needed from Vadim: the two X8 spend decisions in the block below (rows
X8-F, X8b); otherwise nothing (one caution in row 13).** The fourth start (twelve-pair checkpoint U12 seed 2,
...
🟢 **2026-09-24: X8 READ — MOVED.** Filling the two zeroed open-interest columns with Binance's
archive history moved the pre-registered score from 0.526 to 0.537 (+0.011, 4σ, every seed
above the control), and the top-2% trades earn +18.2 gross bps against +10.9 — the first M2
lever to move since 15m → 5m; expectation was FLAT (NEXT_TRAINING_PLAN §2 X8, result block).
Plateaus held (22–26 epochs), so the memorisation signature belongs to *added* columns, not to
any input change. **Nothing served changes on this read.** Both follow-ups were **funded by Vadim the same day**
and are registered and built. **Needed from Vadim: one decision before either launches — (1)
run X8-F exactly as X8 and document a one-bar open-interest skew, or (2) re-run X8 first
under the served convention (three runs) and fold-certify that — row X8-F; Claude recommends
(2). Then launch X8-F's twelve runs serially per WALKFORWARD_PROTOCOL §10.5 and X8b's three,
bringing back the logs.** The X8b identity check passes at its original bar once the archive's
bucket-start labels are aligned to the collector's (the (A) amendment Vadim chose was
withdrawn as unnecessary). Tier 1 texture on the X8 checkpoints was run (pooled +11.6, P2/P3
fail like U12 — decides nothing). Found while building X8b: **the staleness caps have never
fired** (pandas 3 microsecond indexes; row below) — fix built as a default-off knob, kept off
for both families so they pair with their banked controls — and **the archive labels buckets
one bucket early** (row below). The forward test and both side arms are untouched. *Background,
2026-09-23:* fetcher, loader union and identity check built in Docker; the ledger reading
tool no longer trips on the side arms (row 1).
...
**Operational rules learned today, so nobody re-learns them:** *(2026-09-24)* the local `ml_trainer` / `ml_inference` images build for **aarch64** on the Mac, and the PyTorch CPU index names that wheel `torch==2.5.1` while the x86_64 one is `2.5.1+cpu` — the tagged pin could never build locally (it looked like a TLS error under the VPN, then a missing-version error without it). `requirements.txt` now pins `torch==2.5.1` untagged and `Dockerfile.train` installs torch from the CPU index first, so the VM's x86_64 serve image still resolves to exactly `2.5.1+cpu` (what it serves today) and the Mac builds the plain CPU wheel. Rebuilt and verified locally: torch 2.5.1, pyarrow 25.0.1, pandas 3.0.6, both Python tests PASS. after a `git pull`, run
...
X5 is built (`SPLIT_EMBARGO`, default off) and closed without a run. X6 (price-path exits) closed at its gate. X7 (book-keyed sizing) passed its exploration gate weakly and is parked to its confirmation date, 2027-03-10. X8 (archive open interest) came back MOVED 2026-09-24 and opens two spend decisions, X8-F and X8b — those two rows are the only ones here that need Vadim.**
...
| **X8** | **Open-interest history inside the training window, from Binance's public archive** | The served model's `oi` / `oi_chg` legacy columns are zero in train, val and serve because the collector's OI starts 2026-07-18, after the train window ends. The public archive (`data.binance.vision`, USDⓈ-M `metrics`, 5m) carries the same series from 2020-09. Fill the two *existing* columns over the whole window; no column added, no serve feature code touched | GPU, serial: 3 runs (~2.5 h, ≈ $1.5 each, ~8 h); X0 is the banked control (same snapshot, same split). Before it: a fetcher, a loader union and an identity check archive-vs-collector on the 07-18 → 09-09 overlap — all CPU, in Docker | the M2 freeze's own reopening condition ("history inside the *training* window"), met by the archive rather than by the collector's calendar; the ≈2027 date was the collector's | 🟢 **READ 2026-09-24 — MOVED** ([NEXT_TRAINING_PLAN §2 X8](./NEXT_TRAINING_PLAN.md), result block). **Needed from Vadim: nothing on this row** (the decisions it opens are rows X8-F and X8b). Plateau-mean LB **0.5367 vs X0's 0.5258, +0.0108**, ≈ 4σ on the registered SE, every seed above the control (0.5341 / 0.5366 / 0.5393 vs 0.5245 / 0.5264 / 0.5266); no fallback fired — plateaus 26 / 22 / 24 epochs. Secondary: pooled cov-0.02 gross on the 240m head **+18.2 bps over 2,509 trades vs +10.9 over 3,231** — earning, not just ranking. All-epoch means differ by only +0.004 because X8 runs longer past its plateau before patience fires; the registered statistic is the plateau mean. Lookahead check on the archive join passes (snapshots ffilled onto `open_time`, ≥ 5 min old at the close; serve reads collector rows only). Runs s1 `20260923T160559Z`, s2 `20260923T204303Z`, s3 `20260924T024247Z`; logs `logs/X8_s{1,2,3}.log`, eval dumps under `ml/train/output/eval_dumps/`. **Claude does next, free:** `m3 validate`, then `M3_ERA=repaired ./scripts/m3.sh -m m3 universe --runs 20260923T160559Z,20260923T204303Z,20260924T024247Z` → `logs/X8_tier1_<date>.log` — texture only, since U12 showed one-split Tier 1 cannot arbitrate a checkpoint swap. *Background:* archive sha8 83c85bd7, identity check PASS on all twelve pairs; expectation recorded FLAT, wrong |
| **X8-F** | **Certify the `ARCHIVE_OI` recipe on the walk-forward folds** | X8 moved on one validation split. The project's only promotion path is the fold protocol (WALKFORWARD §3, §5.1: W1–W5, twelve runs, the banked fold family as control). A fold run with `ARCHIVE_OI` set departs from the banked recipe, so it needs its own pre-registration and `ALLOW_RECIPE_DRIFT=1`; `SPLIT_EMBARGO` stays off so the control pairing holds | GPU, serial: 4 folds × 3 seeds = 12 runs, 40–55 h wall clock, ≈ $18; CPU read after | X8's licence list (ii), written 2026-09-23 before the read | 🔵 **FUNDED 2026-09-24 (Vadim: yes) — REGISTERED as [WALKFORWARD_PROTOCOL §10](./WALKFORWARD_PROTOCOL.md), harness built, no valid run yet. DECIDED the same day, Vadim: option (2) — X8 is re-run first under the served convention (X8′, `ARCHIVE_OI_SHIFT_MIN=5`, three seeds), then the twelve folds carry the same knob (§10.1). Needed from Vadim, in order: (a) stop the unshifted F2 s1 attempt `20260924T122208Z` if it is still alive (`tmux kill-session -t fluxtrain` on `fluxtrader-train`, then delete that VM — Claude was not permitted to stop a live run) — it is void, nothing trained; (b) launch X8′'s three seeds serially (NEXT_TRAINING_PLAN §2 X8, the X8′ paragraph) and bring back `logs/X8p_s{1,2,3}.log`; Claude reads X8′ against X0; *2026-09-25:* seeds 1 and 2 DONE (`logs/X8p_s1.log`, `logs/X8p_s2.log`, unread); seed 3 FAILED at image build — SQLAlchemy 2.1.0 released between the builds and drops psycopg2 as the default driver (NEXT_TRAINING_PLAN §0.5 trap 13); pinned `<2.1`, **seed 3 to be relaunched by Vadim with the same command**, log as `logs/X8p_s3.log`; (c) only then, **Then: launch the twelve runs, serially, exactly as §10.5 lists them (`gcp_train.sh` with the fold variables, `ARCHIVE_OI`, `ALLOW_RECIPE_DRIFT=1`, `DUMP_MAX_AGE_MIN=100000`; order F2, F3, F1, F0), and bring back each log as `logs/WFX8-<fold>-s<seed>.log`.** Claude verifies §10.6's eleven checks per log, records the run in `dumps.WALKFORWARD_X8_RUNS` / validate's table, fetches the eval dump, and reads §10.3 A (certification, §3's W1–W5) and B (the contrast against the banked family) only once all twelve exist, in a fresh session. A pass promotes **X8 s2** (`m2_multi_20260923T204303Z_d88f3506.pt`, chosen now) through the U12 runbook (row 10) as the forward clock's fifth start; any other outcome promotes nothing. Go/no-go before each VM: the launcher prints one drift item, `ARCHIVE_OI`, and `fold drift accepted`; a `cache miss` means the snapshot moved — stop. `SPLIT_EMBARGO` and `ALIGN_AGE_FIX` stay off on this family (pairing with the banked folds). Era `walkforward_x8` in `m3` (`validate` / `folds` / `folds --contrast walkforward`, all refusing until complete; the banked era re-verified today: TEST 3 PASS, CONFIRMED W1 +33.23) |
| **X8b** | **The flow ratios as a new 4-column feature group** | `ls_global`, `ls_top`, `taker_ratio`, `has_flow` from the same archive, served from the collector's `long_short_ratios` (5m since 2026-08-24); text fixed 2026-09-23 in NEXT_TRAINING_PLAN §2 (X8b) so the decision cannot shape it. Control = X8. Precedent against it: X1 (five added external columns → memorised by epoch 6–7); precedent for it: X8's plateaus held with a filled column | GPU, serial: 3 runs ≈ $4.5, ~8 h; before launch an identity check of the archive ratios vs the collector's over 2026-08-24 → 09-09, and the serve-side 23-column layout in `features.py` / `serve.py` / the checkpoint guard | X8's licence list (iii): "plateaus ≥ 15 with a filled column → X8b is worth running"; they came out 22–26 | 🔵 **FUNDED 2026-09-24 (Vadim: yes) — BUILT, IDENTITY PASS at the original bar; launch waits on the X8-F decision (row above), because that decision fixes X8b's control. Needed from Vadim: nothing beyond the X8-F choice; then launch the three runs.** The `flow` group, loader, serve binding, tests and `./scripts/archive_flow_check.sh` are built (Docker; nothing read). The identity acceptance first **failed** at p99 on 5 of 36 series (2.05–3.14% vs 2%); Vadim answered (A) (amend to 3%); the diagnosis then showed a one-bucket label offset, not noise — re-labelled +5 min the two account series match the collector to **0.011% median, 0.02–0.03% p99 on all twelve pairs**, so the bar stands unamended and the loader applies the re-labelling (NEXT_TRAINING_PLAN §2 X8b amendment 2, §0.5 trap 12). Also found: `ls_top` must be the *account* ratio (the collector has no position ratio; amendment 1), and it is absent in the archive for the first ~136 days of 2022 on nine pairs (a note). Launch: the X8b command block, with `ARCHIVE_OI_SHIFT_MIN` matching the X8-F decision (amendment 4); bring back `logs/X8b_s{1,2,3}.log`; Claude reads them in a fresh session under X8's gate against the control that decision names. Recorded expectation unchanged: WORSE or FLAT with short plateaus |
| **Archive bucket labels are one bucket early** | Binance's public archive stamps a 5-minute bucket with its start and stores the end-of-bucket value; the collector stamps account ratios with the exchange's end timestamp and open interest with its poll time. Unshifted the account ratios disagreed by one bucket's move (p99 1.3–3.1%); re-labelled +5 min they are identical (p99 0.02–0.03%). Open interest matches 3–4× tighter shifted. So **X8 trained on OI one bar fresher than serving supplies** — a small train/serve difference on a slow variable, found after X8's read and before any fold run | loader re-labels the account ratios unconditionally (`ARCHIVE_FLOW_ACCOUNT_SHIFT_MIN=5`); OI under `ARCHIVE_OI_SHIFT_MIN` (default 0 = X8 as run; 5 = served convention), in checkpoint meta, launcher drift; `archiveoi check --shift-min`, `check-flow --raw`; test `test_archive_flow_shift` | NEXT_TRAINING_PLAN §0.5 trap 12; `logs/X8b_identity_flow_20260924.log` | 🟡 **BUILT 2026-09-24. Needed from Vadim: the X8-F decision (that row) — it is this row's only open item.** X8's MOVED read is unaffected as a measurement; what it changes is whether the checkpoint promoted through X8-F is the one whose training matched serving |
| **Staleness caps never fired** | `features._align_with_age` divides `.asi8` by 6e10 (assumes ns); every image since T1 and the VM's inference run pandas 3.0.x with microsecond indexes, so ages are 1000× too small and `BOOK_MAX_AGE_MIN` / `TRADES_MAX_AGE_MIN` / `FUNDING_OI_MAX_AGE_MIN` never trip — `has_*` masks are 1 from a source's first row onward whatever the gap. Uniform across training and serving (same code, same pandas), so no train/serve skew; small live exposure (book/trade columns are zero-forced anyway; funding/OI rarely gap) | fix built 2026-09-24 as `ALIGN_AGE_FIX=1` (config.py; in checkpoint meta, bound at serve from meta, on `/health`, launcher passthrough, drift-reported, refused on folds without `ALLOW_RECIPE_DRIFT=1`); regression test pins both behaviours | found by X8b's synthetic test, which asserted minutes and got thousandths; NEXT_TRAINING_PLAN §0.5 trap 11 | 🟡 **BUILT, DEFAULT OFF. Needed from Vadim: nothing now.** Stays off for X8-F and X8b (both pair against banked controls). **Revival trigger:** goes ON together with X5's `SPLIT_EMBARGO` in the first family whose control is retrained from scratch, as one registered recipe change. Not to be done: switching it on for one arm of any running comparison, or on the served path without a retrain |
...
| **M2 experiments** | **Frozen as a research object** for everything closed on measurement (§5's rows). Eleven levers tested one at a time; two ever moved — 15m→5m and, on 2026-09-24, **X8's archive open-interest fill**. 🔴 **Do not queue another M2 experiment without new *data*** — X8 is what "new data" looks like: history inside the training window for a column the model already has. Open under X8's licence only: X8-F and X8b (experiment table) | [NEXT_TRAINING_PLAN.md](./NEXT_TRAINING_PLAN.md) §2, §5 |
...
For **open interest** the condition is met now — **X8** above was the one lever registered under
it, and it **moved** (read 2026-09-24). For the ratios it is met but adds columns (X8b, now a
decision for Vadim since X8's plateaus held). For the **book** it is
