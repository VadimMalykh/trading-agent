...
them — they are offline.* **Needed from Vadim: pick which to fund next. X1 (WORSE) and X3 (gate not passed) are done,
both 2026-09-15. Recommended next: X4 (CPU-only, the side-table's exits — the one policy
lever never scored), then X2 only if GPU spend is still wanted (its 3-seed control X0 is
banked). Each needs its pre-registration written before a number is read.**
...
| **X4** | **Dynamic exits from the side-table** | Every hold is a fixed 240 minutes. M3-0b's price-path side-table makes signal-flip exits, hold extension while the signal persists, trailing stops and regime-conditional barriers scorable offline; M3_0B_RESULTS lists exactly these as untested. Gate must be net of the extra crossing. | CPU only | M3_0B_RESULTS §C4b: six fixed barriers all lost to the 4h hold, "trailing stops, vol-scaled bands and regime-conditional barriers stay untested" | 🔵 **REGISTERED 2026-09-15 — [WALKFORWARD_PROTOCOL §9.7](./WALKFORWARD_PROTOCOL.md)**, chosen by Vadim. Scoped to the **signal-conditioned** half (flip exits at hourly marks, hold extension while the signal persists; four configurations), scored on the folds from the dumps alone — four 60m returns compound to the 240m return, so no side-table is needed. Statistic: net P&L per seed-day, arm − incumbent; explore F0+F1, confirm F2+F3 once. ⚠️ Trailing stops / vol bands / intrabar barriers stay parked: the side-table starts 2025-11-15 and does not cover the folds. **Needed from Vadim: nothing — Claude builds the harness, runs `validate` and the explore stage, and records the read in §9.7.** |
