...
🟢 **Mode as of 2026-09-21: build while the paper clock runs. Needed from Vadim: a
trading-enabled Binance key + ~$500 in the futures wallet, whenever he wants real orders on
(row 13) — nothing else.** The fourth start (twelve-pair checkpoint U12 seed 2,
...
real-money micro-pilot, built and OFF (row 13). Claude reads the ledger at every 50 closed
...
| 12 | **Exploratory paper arm `explore_cov05`** | `apps/fluxtrader/lib/fluxtrader/trading/policy_engine.ex` | 🔵 **BUILT 2026-09-21; deploy state in the "Mode" paragraph above. Needed from Vadim: nothing.** The policy's own rule (same ladder, same 4 h hold, same costs) at the **top-5% cut 0.6179307699203491** (forward.py's `EXT_CUT`, same checkpoint split) instead of the top 2%. Paper, never risk-checked. **Why:** in a calm market the registered arms are silent for days and the UI shows nothing; eight bars cleared this cut on day one. 🔴 **Not evidence and not a registration**: it overlaps the policy arm, its cut was chosen for visibility, and no reading (R0–R5) touches it. If it ever looks interesting, that licenses *writing* a registration (the parked "re-pre-register the served coverage" row), never a served change. Shown on the dashboard's arm table and `/api/health` → `side_arms` |
| 13 | **Real-money micro-pilot, arm `live`** | [REAL_MONEY_TRACK.md](./REAL_MONEY_TRACK.md) §7 | 🟡 **BUILT 2026-09-21, OFF. Needed from Vadim: a Binance key with futures-trading rights (no withdrawals) in the VM's `.env`, ~$500 USDT in the futures wallet, and the word to turn it on; Claude then runs §7's four steps.** Mirrors every opened paper `policy` trade with a real $100 order at 1x, max 4 open, $10/day and $100 total loss limits (all env vars; budget $500 decided 2026-09-21, to be revisited). The executor stays in `simulation`, so the registered A/B stays paper — this is what replaces row 3's "`auto` on the VM" for a first real trade. Operations test only: ~$0.33 expected per trade; BTC is skipped at $100 (exchange minimum). 140 tests, 0 failures |
