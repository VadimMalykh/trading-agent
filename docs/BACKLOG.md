...
them — they are offline.* **Needed from Vadim: pick which to fund next. X1 is done (WORSE, 2026-09-15).
Recommended order: X3 → X4 (both CPU-only, no M2 change, where the measured effects live),
then X2 only if GPU spend is still wanted, after X0 s3 is re-run. Each needs its
pre-registration written before a number is read.**
...
| **X3** | **Predicted-magnitude sizing** | The ladder — M3's largest measured effect, +8.6 bps on the worst window — keys on *backward-looking* BTC realised volatility. Fit a small tree model for the forward absolute 4h move on candle features over the full history and feed it as the harness's `size_col`; a paired comparison on the same entries against the realised-vol ladder. Never tried as a model. | CPU only, `ml_analysis` container; no M2 change | B1: book and candle features predict *magnitude*, not direction (VOL-PROXY, BOOK_ERA_PLAN §R.1); M3_PROTOCOL §5's bar that magnitude belongs in the policy, not the loss | 🔵 **REGISTERED 2026-09-15 — [WALKFORWARD_PROTOCOL §9.6](./WALKFORWARD_PROTOCOL.md)**, chosen by Vadim. Same entries as the incumbent, same ⅓..5⁄3 quintile ladder, only the key changes: an out-of-fold LightGBM prediction of the pair-normalised forward 4h magnitude from 13 fixed, dump-derived observables. Statistic: X3 − incumbent per unit of notional, day-clustered; explore F0+F1, confirm F2+F3 once. Expectation recorded: flat-to-small-positive, not detectable at confirmation. **Needed from Vadim: nothing — Claude builds the harness, rebuilds the analysis image, runs `validate` and the explore stage, and records the read in §9.6.** |
