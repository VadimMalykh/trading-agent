...
|---|---|---|---|
| 1 | **The forward paper test** | [M3_5_INTEGRATION.md](./M3_5_INTEGRATION.md) | 🔵 **RUNNING since 2026-09-11 04:17 UTC — the second restart, on closed candles with one decision per bar** (M3_FIDELITY_RESULTS §7.6; the 09-10 04:50 start and its six trade rows are void and backed up on the VM as `~/*_formingbar_20260911.csv`). Mode `simulation`, corrected fee. Check on the VM: `curl -s localhost:4000/api/health \| jq '{policy, ab, exec_cost}'` — expect `checkpoint_bound: true`, `served_closed_bars_only: true`, both candle-interval fields `5m`, `fee_tier_verified: true`, and `skips.bar_already_recorded` climbing about 9× faster than `below_coverage` (that is the once-per-bar rule working). **First trade under the registered rule: ZECUSDT long, bar 2026-09-11 04:15, conf 0.6658, regime 0.0177 (calm bucket, policy size 1.33), opened 04:20:18 at 1071.80**, and it is the bar the ledger recorded. One trade is one trade; the split by frozen p80 (row 7) starts when there are tens. Acceptance on the first 42 rows: 32 exact, 10 on not-yet-final bars (row 9) — the run is not void, the rows are the scores the policy acted on. 🔴 Do not change anything on the VM while it accumulates |
...
| 9 | **A closed bar is admitted before its stored row is final (collector polls once a minute)** | [M3_FIDELITY_RESULTS.md](./M3_FIDELITY_RESULTS.md) §7.6 "Acceptance" | 🟡 **FOUND 2026-09-11 05:00 UTC, NOT DEPLOYED — Vadim's call (a VM change, and whether the clock restarts a third time).** Measured at the 04:55 close: the 12 stored rows settle 7–93 s after `close_time`; serve's `close_time <= now` admits the last in-bar poll of an already-closed bar, so ~1 live row in 4 is scored on a bar missing its final seconds to a minute (|Δconf| 0.0002–0.003; no re-decision, the once-per-bar rule holds). Two fixes in §7.6: (a) serve-side `close_time <= as_of − 120 s` (one line, no app rebuild, +≤2 min signal age) or (b) the collector stores/marks only final bars for ≥5m (no lag, app change). Re-check after either: `ml/train/output/probe/accept_76.py` on a day of rows, 42/42 exact is the bar |
