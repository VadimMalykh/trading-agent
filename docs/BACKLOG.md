...
| 2 | **The walk-forward fold queue** — 12 serial runs, F2 first | [WALKFORWARD_PROTOCOL.md](./WALKFORWARD_PROTOCOL.md) §5 | 🔵 harness built and committed; **2 of 12 banked** (F2 s1 `20260905T164940Z`, F2 s2 `20260906T034840Z` — recorded 2026-09-06, all six §5.1 checks pass and `M3_ERA=walkforward m3 validate` TEST 3 reproduces both logs digit-exact). Five earlier runs void (§6.1). The 2026-09-04 attempt inherited the M2-era defaults (8 pairs, 1m, seq 128) from the gitignored `scripts/gcp_env`; §5 now carries the full recipe and `gcp_train.sh` refuses a fold that does not match the incumbent. The four 2026-09-05 runs (F2 s1–s3, F3 s1) trained correctly but were **scored on the wrong window** — `eval_m2.py` always took the newest `VAL_FRACTION` of history, so every fold was measured on F0's rows; fixed 2026-09-05 (`split_from_meta` reads the window off the checkpoint meta) and §5.1 gained a sixth check that compares the eval window to the training `Split` line. **Next: F2 s3 (`VAL_OFFSET=0.250 SEED=3`), then F3, F1, F0** |
