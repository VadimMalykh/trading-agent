...
🔵 **2026-09-23: one offline lever registered — X8; funded, s1 and s2 DONE, s3 running since
2026-09-24 02:42 UTC, nothing read** (experiment-ideas table below; NEXT_TRAINING_PLAN §2). The M2 freeze's reopening condition — book/flow history inside the training window, written
...
Vadim: nothing** — the three runs were funded and Claude reads them in a fresh session once s3 is
DONE (row X8). The fetcher, loader and identity check were built in Docker, reading no model number. The forward test and both side arms
