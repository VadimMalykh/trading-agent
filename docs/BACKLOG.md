...
|---|---|---|---|
| 1 | **The forward paper test** | [M3_5_INTEGRATION.md](./M3_5_INTEGRATION.md) | 🔵 **RUNNING since 2026-09-11 06:07:51 UTC — the third restart, on closed candles that have settled for 120 s, one decision per bar** (M3_FIDELITY_RESULTS §7.6 and its "Acceptance"; the 04:17 start and its two trade rows + 273 bars are void and backed up on the VM as `~/*_settle_20260911.csv`, the 09-10 start as `~/*_formingbar_20260911.csv`). Mode `simulation`, corrected fee. Check on the VM: `curl -s localhost:4000/api/health \| jq '{policy, ab, exec_cost}'` — expect `checkpoint_bound: true`, `served_closed_bars_only: true`, both candle-interval fields `5m`, `fee_tier_verified: true`, `skips.bar_already_recorded` climbing about 9× faster than `below_coverage`; and `curl -s localhost:8001/health \| jq .candle_settle_s` = 120. Acceptance of this start: `ml/train/output/probe/accept_76.py` on a day of rows (needs the VM's candles, `funding_rates` and `open_interest` upserted into the scratch DB), 42/42 exact is the bar. No trade yet under this start. The split by frozen p80 (row 7) starts when there are tens. 🔴 Do not change anything on the VM while it accumulates |
