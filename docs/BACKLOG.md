...
| **The M3 dashboard panel** | [M3_UI_PLAN.md](./M3_UI_PLAN.md) | ⬜ **SPECIFIED 2026-08-29, not started.** The forward paper test is the phase's only deliverable and it is **invisible in the UI** — the dashboard is still the M2-era one, so the only way to see warm state, the A/B arms or the skip reasons is to SSH the VM and read `/api/health` JSON. Read-only, touches no trading logic, one file plus a test. The spec carries the design principle that matters: **the panel's job is to make correct silence legible**, because a system that is quiet for weeks because the market is calm looks exactly like a broken one |
