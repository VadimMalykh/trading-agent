...
and are registered and built: **Needed from Vadim: (1) launch X8-F's twelve fold runs
serially per WALKFORWARD_PROTOCOL §10.5 and bring back the logs (row X8-F); (2) answer the
X8b identity question, (A) amend or (C) void, before X8b is launched (row X8b).** Tier 1
texture on the X8 checkpoints was run (pooled +11.6, P2/P3 fail like U12 — decides nothing).
Found while building X8b: **the staleness caps have never fired** (pandas 3 microsecond
indexes; row below) — fix built as a default-off knob, kept off for both families so they pair
with their banked controls. The forward test and both side arms are untouched. *Background,
