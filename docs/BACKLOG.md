...
🔴 **Mode as of 2026-09-20: the defect is still open. U12 ran, was read, and FAILED Tier 1
(P2 and P5; pooled +4.95, seeds −1.85 / +24.35 / −4.83). Going back to eight pairs is
REJECTED — twelve is the decided universe. Needed from Vadim: yes/no on promoting U12 seed 2
under fold certification (row 10).** The served checkpoint
knows eight pairs and the live path serves twelve, so four instruments trade through an
untrained embedding row; they produced 77% of the bars above the cut and 7 of the 12 closed
trades. The forward ledger keeps accumulating meanwhile, but what it measures is in question
until row 10 resolves. Ask in any session and Claude reads health, and the ledger at every 50
closed trades (row 1).
...
| 10 | **The served checkpoint knows 8 pairs; the live path serves 12** | [NEXT_TRAINING_PLAN.md](./NEXT_TRAINING_PLAN.md) §2 (U12) | 🔵 **OPEN, FOUND 2026-09-16; U12 READ 2026-09-20. Needed from Vadim: yes/no — promote U12 seed 2 (`20260916T164212Z`) as the twelve-pair served checkpoint, certified by the walk-forward folds instead of one-split Tier 1. Claude does the whole promote after a yes.** Twelve pairs stays the served universe either way; the registered "serve eight" fallback is rejected (Vadim, 2026-09-20). Facts (NEXT_TRAINING_PLAN §2 result block, `logs/U12_tier1_20260920.log`): the family failed one-split Tier 1 on P2 and P5 (pooled +4.95 [−41.6, +51.5]; seeds −1.85 / +24.35 / −4.83). Why that bar is contested: T6 measured the incumbent itself failing it in 98.7% of resamples, and RETRAIN_PLAN §8 Q2 (B) already made the folds the certification path — CONFIRMED on twelve pairs, this recipe (W1 +33.23, LB +9.28). **After a yes, in order:** C13 gate + C4 ladder from seed 2's own split → `gcp_promote.sh --checkpoint` → `policy.ex` constants → `/predict` ceiling fix + test (row 11) → deploy, verify health shows 12 served and the new checkpoint bound → back up and void `paper_trades` + `policy_bars` → fourth clock start → `accept_76.py` replay → restate R0–R5. **After a no:** the only twelve-preserving interim is to leave the stack as is (OOV defect open) until another twelve-pair family exists <br><br>*Background — the defect and the launch commands as they were run:* `serve.py:314`'s T5 ceiling is applied only on `/predict_all` (`serve.py:636`); `signal_engine.ex:146` calls `/predict?symbol=` per pair, which has none — verified live (`/predict_all` → 8 pairs, `/predict?symbol=XRPUSDT` → a prediction). ADA/AVAX/LINK/XRP therefore run through `pair_oov_id`, an embedding row no pair trained. On the forward test's own 17,349 bars: **84 of 109 above-cut bars (77%) and 7 of 12 closed policy trades** are those four pairs; XRP alone is 50; five of the eight *trained* pairs never cleared the cut. Decided 2026-09-16: **promote a twelve-pair checkpoint** — the folds already train on twelve (`WALKFORWARD_PROTOCOL` §5.1 check 5), so W1's CONFIRMED +33.23 is a twelve-pair result and the served model is off-recipe. Fresh seeds, not O8/T1/T2 (all three predate the candle repair). Selection rule, acceptance bar and the Tier-1-failure fallback are all fixed in §2 **before any number exists**. <br><br>**The commands, from the laptop:**<br>`# 1. one fresh snapshot, shared by all three`<br>`gcloud compute ssh fluxtrader-1 --zone me-central1-b --project fluxtrader --command "rm -f /var/tmp/fluxtrader_dump_cache.sql.gz"`<br>`# 2. the env block, once per shell`<br>`export FEATURE_GROUPS=legacy CANDLE_INTERVAL=5m PAIR_EMBED_DIM=8 EARLY_STOP_PATIENCE=20`<br>`export TRAIN_HORIZONS=60,240,1440 TRAIN_PRIMARY=240`<br>`export TRAIN_PAIRS=BTCUSDT,ETHUSDT,SOLUSDT,DOGEUSDT,WLDUSDT,HYPEUSDT,ZECUSDT,1000PEPEUSDT,ADAUSDT,AVAXUSDT,LINKUSDT,XRPUSDT`<br>`export DUMP_MAX_AGE_MIN=100000`<br>`# 3. three runs, SERIAL — wait for DONE before the next`<br>`SEED=1 ./scripts/gcp_train.sh --gpu 60 384`<br>`SEED=2 ./scripts/gcp_train.sh --gpu 60 384`<br>`SEED=3 ./scripts/gcp_train.sh --gpu 60 384`<br>`# 4. after each DONE`<br>`./scripts/gcp_status.sh`<br>`./scripts/gcp_logs.sh <run_id> > logs/U12-s1.log   # then U12-s2, U12-s3`<br>`gcloud storage cp gs://fluxtrader-train-artifacts/eval/<run_id>/eval_preds.parquet ml/train/output/eval_dumps/eval_preds_<run_id>.parquet`<br><br>🔴 **Before the VM is created, read the launcher's `recipe:` line** — it must say the incumbent exactly. 🔴 **`Training pairs: [...]` must list twelve** and `Pair embedding: ON dim=8 n_pairs=12`; an 8-pair log is a void run. ~2–3 h and ≈$1.5 per run, ~8–10 h serial. **Claude does everything after the logs come back** — the read, Tier 1, the constants, the promote and the clock restart, each with its own confirmation |
| 11 | **`/predict` has no training-universe ceiling** | [NEXT_TRAINING_PLAN.md](./NEXT_TRAINING_PLAN.md) §2 (U12) | 🟡 **PARKED behind row 10 — Needed from Vadim: nothing.** Row 10 closes the *symptom* by making the checkpoint know all twelve. The *latent bug* stays: `/predict?symbol=` will serve any symbol through `pair_oov_id`, so the next time a checkpoint's universe is narrower than the whitelist this recurs silently. Fix is one line at `serve.py` — refuse a symbol outside `_trained_pairs()` on `/predict` as well — plus a regression test. **Revival trigger: bundle it into row 10's promote**, since that deploy already restarts inference; it must not be a separate live change while the clock runs |
