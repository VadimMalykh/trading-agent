...
any input change. **Nothing served changes on this read.** **Needed from Vadim: two spend
decisions, rows X8-F and X8b below** — (1) certify the `ARCHIVE_OI` recipe on the walk-forward
folds (12 GPU runs, ≈ $18, 40–55 h serial), the only path that can promote it; (2) run X8b, the
flow-ratio columns (3 runs, ≈ $4.5). Claude runs the free Tier 1 texture (`m3 universe`) on the
X8 checkpoints next, and writes the fold pre-registration if (1) is a yes. The forward test and
both side arms are untouched. *Background, 2026-09-23:* fetcher, loader union and identity
check built in Docker; the ledger reading tool no longer trips on the side arms (row 1).
