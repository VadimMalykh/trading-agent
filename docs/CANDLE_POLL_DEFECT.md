...
⚠️ **2026-09-11: no longer unmeasured — the forward test's first trade fired on it.** The 16:35 ZEC bar was scored 0.627 / 0.637 / 0.627 at three prices inside the bar (engine ticks every 30 s) and the trade opened on the middle draw while `policy_bars` recorded the first, un-gated one. Indexed as BACKLOG row 8 with the two-part fix (drop the forming candle; decide once per `bar_ts`) — ✅ fixed and deployed 2026-09-11 04:17 UTC. The acceptance replay then found a smaller residual: the collector polls once a minute, so a closed bar's stored row is refreshed 7–93 s after `close_time` and serve can score the last in-bar snapshot of it — ✅ also fixed the same day (serve admits a bar only 120 s after its close; BACKLOG row 9, M3_FIDELITY_RESULTS §7.6 "Acceptance").
...
## §8 — The backlog's account of the defect, and the repair queue as it was ordered

*Moved here from `BACKLOG.md` on 2026-09-04 (RULES_REVIEW §6.3). The repair is done; this is kept because the ordering decisions it records are the reason the repair was sequenced the way it was.*
...
| ~~**The forming candle is the newest timestep at serve time**~~ ✅ **FIXED 2026-09-11** (`load_candles_tail` takes closed candles only; the app guard requires `closed_bars_only: true`; the engine decides each bar once — M3_FIDELITY_RESULTS §7.6) Residual (BACKLOG row 9): a bar whose `close_time` has passed can still be the last in-bar poll for up to ~93 s; ~1 live row in 4 was scored on it, |Δconf| ≤ 0.003 — ✅ fixed 2026-09-11 06:06 UTC, serve waits `CANDLE_SETTLE_S` = 120 s after the close |
