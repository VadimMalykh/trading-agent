...
|---|---|---|
| 2026-09-20 | **fluxtrader2: P0, P0b, P1, P2 done — PLAN P2 has the plain-language result and the verdict per bet.** Not shown to trade profitably yet, but one bet is funded for building: **a pair's own move over the next 4 hours, by short-term reversal**, traded only when predicted volatility is high. It is far outside the noise floor (200 label shuffles), a fitted forecast reproduces it out of sample (IC 0.035–0.057 against a bar of 0.047 taker / 0.027 maker / 0.024 with volatility timing), and it wants short refits (30–120 days), not more history or a bigger model — a boosted tree never beat a straight line. 15m and 1h are real but below cost; the pair-vs-basket bet did not hold up at 4h–1d (parked with a cheap re-test). **Next session: P3 (the backtest harness), then P4 registered before it runs** — PLAN P2 "Next session"; the work VM is stopped. | **fluxtrader2 needs from Vadim: nothing.** Optional, unchanged: a little BNB in the futures wallet turns the already-enabled 10 % fee discount on (taker round trip 10 → 9 bps). |
