...
|---|---|---|
| 2026-09-21 | **fluxtrader2: P0–P3 done, P4 stage 1 read — PLAN P3/P4 have the plain-language results.** Not shown to trade profitably yet, but the first simulated ledger is positive. The backtest harness (`ft2 backtest`) exists and reads true: a coin-flip strategy on real data loses exactly the measured cost (−12.2 bps taker, −7.2 maker; a bps is 0.01 %). Through it, the fixed no-model rule for the funded bet (**fade a pair's last 4h–1d move when it is volatile, hold 4 hours**; registered as R1 before it ran) made 3,404 trades on 2023-05 → 2024-08, right 57 % of the time, **+14 bps per trade after costs as a maker (+14 USDT per 10,000 USDT trade, ~+100 USDT a day)**, and no shuffle of the data came close. Cautions: the error bar is ±17 bps (trades bunch on volatile days), **all the profit is on the long side** (longs +51, shorts −19 — part of it is buying dips in a rising market), and the stricter null gives p 0.03. **Next session (Claude): P5 aimed at cutting that spread, the side-flip null, and the parked market-neutral rank rule** — PLAN §7. The work VM is stopped. | **fluxtrader2 needs from Vadim: one choice — when to spend a confirmation fold on this rule (PLAN §7, first row).** Recommended (a): not yet — F3 alone would confirm a true +14 only one time in five; improve the rule on the exploration folds first, then one powered read. Saying nothing = (a). Optional, unchanged: a little BNB in the futures wallet turns the already-enabled 10 % fee discount on. |
