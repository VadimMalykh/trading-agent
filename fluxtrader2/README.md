...
|---|---|---|
| 2026-09-21 | **fluxtrader2: P0–P4 done, P5 steps 1–3 read — PLAN P5 has the plain-language result.** Not shown to trade profitably. Three things happened today. **(1) A defect:** the statistic P2 used to measure signals reads a false "reversal" on pure random data; fixed and re-run — the pair-level 4-hour reversal that P2 funded and P4's rule was built on **is not there** (IC, the correlation between signal and next move, −0.045 → −0.013). Money results from the backtest harness never used that statistic and stand. **(2) The market itself, 4.3 years, per year:** one feature keeps its sign in every year — after a sharp 4-hour fall the market bounces *if its 30-day trend is up* (+54 bps over the next 4 hours before costs; a bps is 0.01 %, so +54 USDT per 10,000 USDT) and not if it is down (+5). As a rule through the harness: **+52 bps a trade after costs [+26, +77]**, ~2 trades a day — but measured on the same data it was found on, pure market timing, and 2022 missed its pre-written bar by 0.9 bps (−5.9 vs −5), so by its own gate it is **parked without touching the untouched data**. **(3) A new lead from the re-run:** how lopsided the order book is within ±1 % of the price predicts a pair's next day (IC −0.039, p 0.005). **Next session (Claude): that book-imbalance signal as a registered rule, then the ~40-pair universe for the "torn-away pair keeps going" hypothesis** — PLAN P5 "Next session". The work VM is stopped. | **fluxtrader2 needs from Vadim: nothing.** One optional decision (PLAN §7, first row): read the three untouched folds (2024-09 → 2026-09) for the parked market-timing rule? Recommendation (a) no, leave it parked; saying nothing = (a). Optional, unchanged: a little BNB in the futures wallet turns the already-enabled 10 % fee discount on. |
