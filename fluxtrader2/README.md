...
|---|---|---|
| 2026-09-20 | **fluxtrader2: P0, P0b, P1 done; P2 (ceiling audit) five of seven items measured — PLAN P2 has the plain-language result.** Not shown to trade profitably yet. Costs (12 bps taker, 7 maker) are small next to moves from 4h up; the size of a move is predictable (R² 5–16 % out of sample), its direction barely (≤ 0.2 %); every surviving signal is short-term *reversal*; the only horizons where a measured signal reaches the cost bar are **4h and 1d**, as candidates only. 15m is excluded for both bets. **Next session: P2 #4 (noise floor) and #5 (learning curves) on the 4h/1d candidates, then the verdict column and P4's bet** — exact commands at the end of PLAN P2; the work VM is stopped. | **fluxtrader2 needs from Vadim: the Binance USDⓈ-M fee tier** (VIP level + BNB discount on/off, or a read-only key) — it moves every bar in the P2 table; `vm.sh run cost --taker-bps X --maker-bps Y --fee-source '…'` then `ceiling` with the same flags re-prices everything. Nothing else. |
