...
|---|---|---|
| 2026-09-21 | **fluxtrader2: P0–P4 done, P5 step 1 read — PLAN P5 has the plain-language result.** Not shown to trade profitably yet. The fixed rule from P4 (fade a volatile pair's last 4h–1d move, hold 4 hours; +14 bps per trade after costs as a maker, i.e. +14 USDT per 10,000 USDT trade; a bps is 0.01 %) was taken apart: **its profit is the whole market bouncing after a violent fall, not the pair reverting** — measured against the other pairs its trades lose 12 bps; buying falls earned +51 in every quarter, shorting spikes lost 19. The market-neutral version of the bet loses outright (−21 bps a leg, every quarter) and is closed; two attempts to make the result less lumpy (smaller bets on wild pairs, at most three positions a side) made it worse and are closed. What is left is two sharper hypotheses — "buy a market-wide panic" and "a pair torn from the pack keeps going" — both found on the data already looked at, so not yet evidence. **Next session (Claude): fetch 2020–2022 candles from the public archive as fresh exploration data and test both there, without touching the confirmation folds** — PLAN P5 "Next session". The work VM is stopped. | **fluxtrader2 needs from Vadim: nothing new.** The one open choice is unchanged (PLAN §7, first row: when to spend a confirmation fold on the P4 rule) and the recommendation is now firmer: (a) not yet. Saying nothing = (a). Optional, unchanged: a little BNB in the futures wallet turns the already-enabled 10 % fee discount on. |
