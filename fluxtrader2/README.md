...
|---|---|---|
| 2026-09-23 | **fluxtrader2: P0–P4 done, P5 steps 1–7 read; step 8 (R13) REGISTERED and RUNNING — PLAN P5 has the plain-language result.** Not shown to trade profitably; every fixed rule has been read and none clears its gate. Today: the plan's own deliverable was built and read — a ridge forecast of a pair's next-day move against its peers, fitted on 30 of the 40 pairs and scored on the 10 it never saw, traded when it clears 15 bps (R12). **It does not survive the hold-out: IC 0.0075 (t 1.3) where the in-pair number on twelve names had been 0.048; the forecast almost never clears cost, and the 1,648 trades it did make are wild names with no statistical power (net −44 [−101, +12]).** Parked. The open question — is the signal pair-specific (does not transfer), or was it the target/universe change — is R13, the same pipeline fitted in pair on the forty and on the twelve, running on the work VM. **Next session (Claude): `vm.sh pull`, read `output/backtest/r13_ridgebook_1d_inpair*/forecast.md`, fill R13's Result, apply its gate, stop the VM** (PLAN §8 R13). | **fluxtrader2 needs from Vadim: nothing.** Optional, unchanged: a little BNB in the futures wallet turns the already-enabled 10 % fee discount on. |
